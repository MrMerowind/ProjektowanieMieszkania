...
namespace ProjektowanieMieszkaniaCSharp
{
    class Wall
    {
        ...
        private Color color = Color.Black; // Color.FromArgb(14, 9, 66);
        ...
        public Wall(PointF _start, PointF _end, float _thickness)
        {
            start = _start;
            end = _end;
        ...
        public Wall(float _startX, float _startY, float _endX, float _endY, float _thickness)
        {
            start.X = _startX;
            start.Y = _startY;
        ...
        {
            PointF wallStart = new PointF();
            PointF wallEnd = new PointF();
            if(isHorizontal())
            {
                wallStart.X = start.X - thickness / 2.0f;
                wallStart.Y = start.Y;
                wallEnd.X = end.X + thickness / 2.0f;
                wallEnd.Y = end.Y;
            }
            else if(isVertical())
            {
                wallStart.X = start.X;
                wallStart.Y = start.Y - thickness / 2.0f;
                wallEnd.X = end.X;
                wallEnd.Y = end.Y + thickness / 2.0f;
            }
            g.DrawLine(new Pen(GetColor(), thickness), wallStart, wallEnd);
        }
        public void drawWithoutAdditionalSomething(Graphics g)
        {
